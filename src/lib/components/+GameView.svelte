<script lang="ts">
    ...
    const restartGame = (saveState: GameState) => {
        ...
        gameboard.flat().filter((cellOrNull: Cell|null)=> cellOrNull ? true : false).forEach((cell: Cell|null) => {
    ...
    const moveCellInDOM = (cell: Cell) => {
        ...
        el?.classList.remove("c-0-0", "c-0-1", "c-0-2", "c-0-3", "c-1-0", "c-1-1", "c-1-2", "c-1-3", "c-2-0", "c-2-1", "c-2-2", "c-2-3", "c-3-0", "c-3-1", "c-3-2", "c-3-3")
...
<!-- OPTIONS -->
<div class="flex flex-row justify-between mb-4 w-64">
    <button on:click={() => initializeNewGame()} class="bg-neutral-300 text-black p-2 text-[12px] sm:text-xs font-bold">NEW GAME</button>
    <button disabled={previousTurn == null} on:click={() => undoTurn()} class={`${previousTurn != null ? "bg-neutral-50" : "bg-neutral-600"} text-black p-2 text-[12px] sm:text-xs font-bold`}>UNDO TURN ({previousTurn == null ? 0 : 1})</button>
</div>

<!-- STATS -->
<div class="flex flex-row justify-between mb-4 text-center w-64">
    <div id="highest-number" class="text-lg sm:text-2xl bg-neutral-600 w-16 sm:w-28 h-10 flex items-center justify-center font-bold">
        {findHighest(gameboard)}
    </div>
    <div class="flex flex-col gap-4 text-right ">

        <div id="score-box" class="">
            <div class="font-semibold text-neutral-500 text-[12px]  leading-3">SCORE</div>
            <div class="font-extrabold leading-6 text-white text-xl">{score}</div>
        </div>
        <div id="turn-box" class="">
            <div class="font-semibold text-neutral-500 text-[12px]  leading-3">TURN</div>
            <div class="font-extrabold leading-6 text-white text-xl">{turn}</div>
        </div>
    </div>
</div>
...
>
    ...
    <div
        id="active-cells"
        class="absolute top-0 left-0 right-0 bottom-0 w-64 h-64"
    >
...
#game-board-wrapper::before{
    ...
    height: 600px;
    z-index: -10;;
    background: radial-gradient(circle at 50% 50%, rgba(0, 209, 255, 1) 0%, rgba(9, 0, 255, 1) 28%, rgba(0, 54, 212, 0) 60%);
}
